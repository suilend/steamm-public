...
module steamm::lend_tests {
    ...
    fun test_simple_deposit_with_lending_a() {
        ...
        let mut btoken_a = bank_a.mint_btokens(&lending_market, coin_a, ctx);
        let mut btoken_b = bank_b.mint_btokens(&lending_market, coin_b, ctx);

        // Test bank effects after minting btokens
        assert_eq(bank_a.funds_deployed(&lending_market).floor(), 0); // 500_000 * 0%
        ...
        assert_eq(bank_a.funds_deployed(&lending_market).floor(), 400_000); // 500_000 * 80%
    ...
    fun test_swap_with_lending_without_touching_lending_market() {
        ...
        assert_eq(bank_a.funds_deployed(&lending_market).floor(), 0);
        assert_eq(bank_a.funds_available().value(), 500_000);
        
        assert_eq(bank_b.funds_deployed(&lending_market).floor(), 0);
        assert_eq(bank_b.funds_available().value(), 500_000);
    ...
    fun test_simple_deposit_with_lending_ab() {
        ...
        assert_eq(bank_a.funds_deployed(&lending_market).floor(), 0); // 500_000 * 0%
        assert_eq(bank_a.funds_available().value(), 500_000); // 500_000 * 100%
        assert_eq(bank_b.funds_deployed(&lending_market).floor(), 0); // 500_000 * 0%
        assert_eq(bank_b.funds_available().value(), 500_000); // 500_000 * 100%
        ...
        assert_eq(bank_a.funds_deployed(&lending_market).floor(), 400_000); // 500_000 * 80%
        assert_eq(bank_a.funds_available().value(), 100_000); // 500_000 * 20%
        assert_eq(bank_b.funds_deployed(&lending_market).floor(), 400_000); // 500_000 * 80%
    ...
    fun test_swap_with_lending_within_utilization_range() {
        ...
        // Test bank effects after minting btokens
        assert_eq(bank_a.funds_deployed(&lending_market).floor(), 400_000); // No change
        assert_eq(bank_a.funds_available().value(), 100_000 + 50_000);
        assert_eq(bank_b.funds_deployed(&lending_market).floor(), 400_000); // No change
        ...
        assert!(!bank_a.needs_rebalance(&lending_market));
        assert!(!bank_b.needs_rebalance(&lending_market));

        assert_eq(bank_a.funds_deployed(&lending_market).floor(), 400_000);
        assert_eq(bank_a.funds_available().value(), 150_000);
        
        assert_eq(bank_b.funds_deployed(&lending_market).floor(), 400_000);
    ...
    fun test_swap_with_lending_beyond_utilization_range() {
        ...
        assert!(bank_a.needs_rebalance(&lending_market), 1);
        assert!(bank_b.needs_rebalance_after_outflow(&lending_market, btoken_b.value()), 2);

        assert_eq(bank_a.funds_deployed(&lending_market).floor(), 400_000);
        assert_eq(bank_a.funds_available().value(), 300_000);

        assert_eq(bank_b.funds_deployed(&lending_market).floor(), 400000);
        ...
        assert_eq(bank_a.funds_deployed(&lending_market).floor(), 560_000);
        assert_eq(bank_a.funds_available().value(), 140_000);
        
        assert_eq(bank_b.funds_deployed(&lending_market).floor(), 286_857); // 286,857.6
    ...
    fun test_deposit_with_lending_all_scenarios() {
        ...
        assert_eq(bank_a.funds_deployed(&lending_market).floor(), 80_000); // 100_000 * 80%
        assert_eq(bank_a.funds_available().value(), 20_000); // 500_000 * 20%
        assert_eq(bank_b.funds_available().value(), 100_000);
        
        assert_eq(bank_a.effective_utilisation_bps(&lending_market), 8000); // 80% target liquidity
        ...
        assert!(bank_a.effective_utilisation_bps(&lending_market) < bank_a.target_utilisation_bps(), 0);
        assert!(bank_a.effective_utilisation_bps(&lending_market) > bank_a.target_utilisation_bps() -  bank_a.utilisation_buffer_bps(), 0);
        ...
        assert_eq(bank_a.funds_deployed(&lending_market).floor(), 4_084_000); // 5_105_000 * 80%
    ...
    fun test_deposit_with_lending_proptest() {
        ...
        while (deposits > 0) {
            ...
            assert!(
                bank_a.effective_utilisation_bps(&lending_market).max(8000) - bank_a.effective_utilisation_bps(&lending_market).min(8000) <= 1
    ...
    fun test_lend_redeem_with_lending_within_utilization() {
        ...
        assert_eq(bank_a.funds_deployed(&lending_market).floor(), 80_000); // amount lent does not change
    ...
    fun test_lend_amm_swap_small_swap_scenario_no_rebalance() {
        ...
        // Confirm that bank DOES NOT need to be rebalanced
        assert!(!bank_a.needs_rebalance_after_outflow(&lending_market, btoken_a.value()), 2);
        assert!(!bank_b.needs_rebalance(&lending_market), 1);
        
        assert_eq(bank_a.funds_deployed(&lending_market).floor(), 80_000);
        assert_eq(bank_a.funds_available().value(), 20_000);

        assert_eq(bank_b.funds_deployed(&lending_market).floor(), 80_000);
    ...
    fun test_lend_amm_swap_medium_swap_scenario() {
        ...
        assert_eq(bank_a.funds_deployed(&lending_market).floor(), 64_000);
        assert_eq(bank_a.funds_available().value(), 16_000);

        assert_eq(bank_b.funds_deployed(&lending_market).floor(), 96_000);
    ...
    fun test_lend_amm_swap_large_swap_scenario() {
        ...
        // Assert rebalancing result
        assert_eq(bank_a.funds_deployed(&lending_market).floor(), 56_000);
        assert_eq(bank_a.funds_available().value(), 14_000);

        assert_eq(bank_b.funds_deployed(&lending_market).floor(), 104_000);
    ...
    public fun test_no_op_below_min_deploy_amount() {
        ...
        let effective_utilisation_bps_before = bank_sui.effective_utilisation_bps(&lending_market);
        ...
        let effective_utilisation_bps_after = bank_sui.effective_utilisation_bps(&lending_market);
