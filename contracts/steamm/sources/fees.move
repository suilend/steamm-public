...
public(package) fun new<A, B>(fee_numerator: u64, fee_denominator: u64, min_fee: u64): Fees<A, B> {
    Fees {
        config: FeeConfig {
            fee_numerator,
            fee_denominator,
...
public(package) fun new_config(fee_numerator: u64, fee_denominator: u64, min_fee: u64): FeeConfig {
    FeeConfig {
        fee_numerator,
        fee_denominator,
        min_fee,
...
public fun fee_ratio<A, B>(fees: &Fees<A, B>): (u64, u64) {
    (fees.config.fee_numerator, fees.config.fee_denominator)
}

public fun fee_ratio_(fee_config: &FeeConfig): (u64, u64) {
...
public fun balances<A, B>(fees: &Fees<A, B>): (&Balance<A>, &Balance<B>) {
    (&fees.fee_a, &fees.fee_b)
}
...
#[test_only]
public(package) fun fee_numerator_mut(fee_config: &mut FeeConfig): &mut u64 {
    &mut fee_config.fee_numerator
}
#[test_only]
