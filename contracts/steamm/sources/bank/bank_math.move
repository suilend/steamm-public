...
public(package) fun compute_recall_for_pending_withdraw(
    ...
    let needs_recall = if (withdraw_amount > funds_available) { true } else {
        let post_utilisation_bps =
            compute_utilisation_bps(funds_available - withdraw_amount, funds_deployed) as u64;
...
fun test_compute_recall_amount() {
    assert_eq(
        compute_recall_for_pending_withdraw(2_000, 0, 8_000, 8_000, 500),
        0,
    );

    assert_eq(
        compute_recall_for_pending_withdraw(2_000, 1_000, 8_000, 8_000, 500),
        800,
    );

    assert_eq(
        compute_recall_for_pending_withdraw(2_000, 2_000, 8_000, 8_000, 500),
        1_600,
    );

    // Does not need recall as it does not change liq. ratio beyond the bands
    assert_eq(
        compute_recall_for_pending_withdraw(2_000, 100, 8_000, 8_000, 500),
        0,
    );
}
