...
module slamm::smm_tests {
    ...
    fun test_smm_trade_max_fees() {
        ...
        assert_eq(swap_result.pool_fees(), 400_000); // hook fees
        assert_eq(swap_result.protocol_fees(), 100_000); // hook fees
        assert_eq(swap_result.amount_out(), 500_000);
    ...
    fun test_smm_trade_min_fees() {
        ...
        assert_eq(swap_result.pool_fees(), 0); // hook fees
        assert_eq(swap_result.protocol_fees(), 0); // hook fees
    ...
    #[test]
    fun test_trade_with_divergent_reserve_ratio_same_side() {
        let mut scenario = test_scenario::begin(ADMIN);
        ...
        let r1 = decimal::from_scaled_val(611111111111111111); // 61%

        let delta = r1.sub(r0);
        print(&delta);
        
        assert_eq(swap_result.a2b(), true);
        assert_eq(swap_result.pool_fees(), 4448); // hook fees
        assert_eq(swap_result.protocol_fees(), 1112); // hook fees
        assert_eq(swap_result.amount_out(), 50_000);
