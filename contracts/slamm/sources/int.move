module slamm::int {
    ...
    public fun sub(a: Int, b: Int): Int {
        if (is_positive(a)) {
            ...
            if (is_positive(b)) {
                // B is positive
                if (a.value >= b.value) return from_u256(a.value - b.value);
                // Return positive
                return neg_from_u256(b.value - a.value)
                // Return negative
            } else {
                // B is negative
                return from_u256(a.value + abs(b).value)
                // Return positive
            }
        } else {
            // A is negative
            if (is_positive(b)) {
                // B is positive
                return neg_from_u256(abs(a).value + b.value)
                // Return negative
            } else {
                // B is negative
                let a_abs = abs(a);
                let b_abs = abs(b);
                if (b_abs.value >= a_abs.value) return from_u256(b_abs.value - a_abs.value);
                // Return positive
                return neg_from_u256(a_abs.value - b_abs.value)
                // Return negative
            }
        }
    }
    ...
    public fun mul(a: Int, b: Int): Int {
        if (is_positive(a)) {
            ...
            if (is_positive(b)) {
                // B is positive
                return from_u256(a.value * b.value)
                // Return positive
            } else {
                // B is negative
                return neg_from_u256(a.value * abs(b).value)
                // Return negative
            }
        } else {
            // A is negative
            if (is_positive(b)) {
                ...
                // B is negative
                return from_u256(abs(a).value * abs(b).value)
                // Return positive
            }
        }
    }
    ...
    public fun div_down(a: Int, b: Int): Int {
        if (is_positive(a)) {
            ...
            if (is_positive(b)) {
                // B is positive
                return from_u256(math256::div_down(a.value, b.value))
                // Return positive
            } else {
                // B is negative
                return neg_from_u256(math256::div_down(a.value, abs(b).value))
                // Return negative
            }
        } else {
            // A is negative
            if (is_positive(b)) {
                // B is positive
                return neg_from_u256(math256::div_down(abs(a).value, b.value))
                // Return negative
            } else {
                // B is negative
                return from_u256(math256::div_down(abs(a).value, abs(b).value))
                // Return positive
            }
        }
    }
    ...
    public fun div_up(a: Int, b: Int): Int {
        if (is_positive(a)) {
            ...
            if (is_positive(b)) {
                // B is positive
                return from_u256(math256::div_up(a.value, b.value))
                // Return positive
            } else {
                // B is negative
                return neg_from_u256(math256::div_up(a.value, abs(b).value))
                // Return negative
            }
        } else {
            // A is negative
            if (is_positive(b)) {
                // B is positive
                return neg_from_u256(math256::div_up(abs(a).value, b.value))
                // Return negative
            } else {
                // B is negative
                return from_u256(math256::div_up(abs(a).value, abs(b).value))
                // Return positive
            }
        }
    }
